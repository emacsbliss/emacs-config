...
The issue with nano-theme is it doesn't look very nice with source code like c++
or python, there are just pretty much one color for different symbols which
makes it looks less appealing than spacemacs-theme
...
#+END_SRC
** ivy occur
When I get into the ivy occur buffer via ~C-c C-o~ after an ivy search,
one of the important keybinding are missing which is to switch to other buffers.

So we need to add it and I bind it to the same keybinding as before:
#+begin_src emacs-lisp :tangle keybinds.el
(map! :map ivy-occur-grep-mode-map
      :desc "ace window" :nv "gw" #'ace-window)
#+end_src
...
(map!
  ...
  :desc  "org roam node find" :nv "ga" #'org-roam-node-find
  ;; org-roam-buffer-toggle is not working for me for some reason
  ;; :desc  "org roam" :nv "gA" #'org-roam-buffer-toggle
  :desc  "org roam" :nv "gA" #'org-roam-buffer
  ...
  ;:nv "gy" #'tldr
  :nv "gy" #'org-agenda-super-view-today
  :nv "gY" #'org-agenda-mega-view
...
(map! :leader
      ...
      :desc "org projectile" "X"           #'org-projectile-project-todo-completing-read
...
(map! :leader
      (:prefix-map ("a" . "applications")
        ...
        :desc "next journal entry"         "n" #'org-journal-next-entry
        :desc "prev journal entry"         "p" #'org-journal-previous-entry
...
(map! :leader
  ...
  (:prefix-map ("f" . "file")
    ...
    :desc "Yank file full path"         "Y"   #'+default/yank-buffer-path
...
(map! :leader
  ...
  (:prefix-map ("n" . "notes")
    ...
    :desc "open GTD inbox"               "i" #'me/open-gtd-inbox
...
(map! :leader
  ...
  (:prefix-map ("o" . "open")
    ...
    :desc "world clock"        "C"  #'world-clock
    :desc "Start debugger"     "d"  #'+debugger/start
    :desc "New frame"          "f"  #'make-frame
    :desc "REPL"               "r"  #'+eval/open-repl-other-window
    :desc "REPL (same window)" "R"  #'+eval/open-repl-same-window
    :desc "rss(elfeed)"        "s"  #'=rss
...
(map! :leader
  ;;; <leader> r --- roam
    (:prefix-map ("r" . "org-roam")
      ...
      ;; find todo items within org-roam
      :desc "org roam todo"                       "o" #'me/org-roam-todo
      :desc "helm-bibtex"                         "x" #'helm-bibtex
...
(map! :leader
  ...
  (:prefix-map ("t" . "toggle")
    ...
    :desc "view mode"                    "v" #'view-mode
    :desc "visual line mode"             "V" #'visual-line-mode
    :desc "follow mode"                  "f" #'follow-mode
    ;; :desc "Flymake"                      "f" #'flymake-mode
    ;; (:when (featurep! :tools flycheck)
    ;;   :desc "Flycheck"                   "f" #'flycheck-mode)
    :desc "Frame fullscreen"             "F" #'toggle-frame-fullscreen
    :desc "Evil goggles"                 "g" #'evil-goggles-mode
    :desc "horizontal scroll bar"        "h" #'horizontal-scroll-bar-mode
...
I think we need ls from gnu:
#+BEGIN_QUOTE
Listing directory failed but 'access-file' worked
#+END_QUOTE

this setting will put directories first:
#+begin_src emacs-lisp :tangle modules/my/dired/config.el :mkdir yes
...
(map! :map dired-mode-map
  ...
  :desc "up directory" :n "h" #'dired-up-directory
  :desc "directory" :n "l" #'dired-find-file
  :desc "copy full path" :n "f" #'diredp-copy-abs-filenames-as-kill
  :desc "kill line" :n ",k" #'dired-kill-line
  :desc "kill lines" :v ",k" #'dired-do-kill-lines
  :desc "toggle details mode" :nv ",d" #'dired-hide-details-mode)
#+END_SRC
** working with remote
Whenever we delete a remote file via tramp, it fails with error like:
#+begin_example
File is missing: Searching for program No such file or directory trash
error: "Failed to trash \"/ssh:me@nas:/volume1/tmp/test.xml\": \"/Users/emacsbliss/.emacs.d/.local/straight/build-28.0.50/osx-trash/trashfile.AppleScript:88:114: execution error: Finder got an error: Handler can’t handle objects of this class. (-10010) \""
if: Failed to delete "/ssh:me@nas:/volume1/tmp/test.xml"
#+end_example

the delete file functionality is bound to function ~doom/delete-this-file~, particularly this line:
#+begin_src emacs-lisp :tangle no
(progn (delete-file path t) t)
#+end_src

Based on the documentation, we are calling it by saying TRASH to t,
because the variable ~delete-by-moving-to-trash~ is set to t in my config.
#+begin_quote
Signature
...
The reason why delete fails is because there is no trash can support in remote system
when we delete file via tramp, so we need to temporarily disable that trash behavior in this case.

Turned out we can use advice to fix this instead of changing this function.
One thing to be careful is we need two advice, one before and one after so that
later if we delete a local file, it will still go to trash instead of deleted,
we only want that behavior when it's remote file which we can use function
~file-remote-p~ to find out.
#+begin_src emacs-lisp :tangle modules/my/dired/config.el :mkdir yes
;;;###autoload
(defun me/tramp-delete-file-advice-before (file &optional _trash)
...
#+begin_src emacs-lisp :tangle modules/my/dired/config.el :mkdir yes
;;;###autoload
(defun me/tramp-delete-file-advice-after (file &optional _trash)
  (when (file-remote-p file)
    (setq delete-by-moving-to-trash t)))
#+end_src

Now we can add those advise to the ~delete-file~ function:
#+begin_src emacs-lisp :tangle modules/my/dired/config.el :mkdir yes
(advice-add 'delete-file :before #'me/tramp-delete-file-advice-before)
(advice-add 'delete-file :after #'me/tramp-delete-file-advice-after)
#+end_src

Turned out we need to do the same thing for directory:
#+begin_src emacs-lisp :tangle modules/my/dired/config.el :mkdir yes
;;;###autoload
(defun me/tramp-delete-dir-advice-before (dir &optional recursive _trash)
...
#+begin_src emacs-lisp :tangle modules/my/dired/config.el :mkdir yes
;;;###autoload
(defun me/tramp-delete-dir-advice-after (dir &optional recursive _trash)
  (when (file-remote-p dir)
    (setq delete-by-moving-to-trash t)))
#+end_src

#+begin_src emacs-lisp :tangle modules/my/dired/config.el :mkdir yes
(advice-add 'delete-directory :before #'me/tramp-delete-dir-advice-before)
(advice-add 'delete-directory :after #'me/tramp-delete-dir-advice-after)
#+end_src
* evil
...
;; make ccls default by lowering the priority of clangd
...
(setq +notes-dir (expand-file-name "~/org/gtd"))
(setq org-default-notes-file (expand-file-name "~/org/gtd/notes.org"))
#+end_src
...
The default value of ~org-link-file-path-type~ is adaptive, which means make
relative links if the link is to the current directory below, otherwise fully
qualify the link.
...
If we don't use relative, then working copy on iOS will fail to render the image.
** convert to org file
...
(use-package! org-re-reveal
  :after ox
  :config
  (setq org-re-reveal-root (substitute-in-file-name "file://$HOME/reveal.js")
...
#+end_src
*** export to pdf
We also need to set this otherwise bib entry won't appear in pdf file:
#+BEGIN_SRC emacs-lisp :tangle modules/emacs/org/config.el
(after! org
...
(map! :map org-mode-map
  ...
  :desc "org gtd finalize"             :n ",f" #'org-gtd-clarify-finalize
  :desc "attach json from killring"    :n ",j" #'me/attach-json-from-kill-ring
  :desc "org attach by mv"             :n ",m" #'org-attach-attach-mv
  ...
  :desc "org schedule"                 :n ",s" #'org-schedule
  :desc "org promote subtree"          :n ",<" #'org-promote-subtree
  :desc "org demote subtree"           :n ",>" #'org-demote-subtree)
#+END_SRC
* org capture
...
For some weird reason, with org-mode 9.5, I'm getting error:
#+begin_example
condition-case: Capture abort: Args out of range: 0, 0
#+end_example

it turned out it works if I do this:
#+begin_src emacs-lisp :tangle no
    (format "%s
...
** capture web page
There are multiple ways to capture a web page into org mode file.
*** org-web-tools
#+begin_src emacs-lisp :tangle modules/emacs/org/packages.el
...
(after! org
  (add-to-list 'org-capture-templates
    `("e" "email(mu4e)" entry  (file "~/org/gtd/inbox.org")
...
* GTD
The GTD system is mostly based on org mode with some tweaks.

Borrowed lots of ideas from [fn:12].
** packages
#+BEGIN_SRC emacs-lisp :tangle packages.el
...
(use-package! org-gtd
  ...
  ; :init
  ;; do NOT use C-c C-c, it has meaning:
...
We don't need this since ~projects.org~ is already in the list of ~org-agenda-files~:
#+begin_src emacs-lisp :tangle no
...
#+end_src
*** layout
Sometimes it could be really hard if you don't have clear idea what and how you want
the things to be shown on your agenda. Once that is figured out, then rest will be
easy to make use of the org-super-agenda. Also some ideas come from [[https://github.com/bascht/dotfiles-public/blob/master/dot_doom.d/org.el][here]]:
#+begin_src emacs-lisp
(setq org-agenda-custom-commands
      '(("u" "Super view"
         agenda (org-super-agenda-mode)
         ((org-super-agenda-groups
           '(
             (:name "\n⚡ Holiday/Birthday/Anniversary:\n⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺"
              :category ("Holidays" "Birthdays" "Anniversary")
              :order 1)
              ...
              (:name "\n⚡ Do Today:\n⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺⎺"
              ;; can't have this, otherwise overdue won't show
              ;; :todo ("TODO" "NEXT")
              :date today
              :order 3)
        ...
        ("b" "Birthday View"
            ((agenda "" (
             ...
             (org-super-agenda-mode)
                        (org-super-agenda-groups
                       '((:name "birthday"
                          :category ("Birthdays" "Anniversary")
                          :discard (:anything t)
                                  :order 1)))))))
...
#+begin_src emacs-lisp :tangle modules/emacs/org/autoload.el :mkdir yes
;;;###autoload
(defun org-agenda-super-view-today (&optional arg)
  (interactive "P")
  (org-agenda arg "u"))

;;;###autoload
(defun org-agenda-mega-view (&optional arg)
  (interactive "P")
  (org-agenda arg "c"))
#+end_src
...
#+begin_src emacs-lisp :tangle modules/emacs/org/config.el :mkdir yes
(after! org
    ;; these fancy characters caused problem with org-gtd package
    ;; (setq org-todo-keywords '((sequence "☛ TODO(t)" "|" "NEXT" "|" "✔ DONE(d)")
    ;;                             (sequence "⚑ WAITING(w)" "|")
    ;;                             (sequence "|" "✘ CANCELED(c)")))
    (setq org-agenda-files '("~/org/gtd")))
#+end_src
*** prettify
Now we need to tweak some settings to make it looks nicer, heavy inspired by [fn:15]:
#+begin_src emacs-lisp :tangle modules/emacs/org/config.el :mkdir yes
(after! org
(setq org-agenda-remove-tags t)
(setq org-agenda-current-time-string "ᐊ┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈ Now")
(setq org-agenda-time-grid (quote ((today require-timed remove-match) (0900 2100) "      " "┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈")))
(setq org-deadline-warning-days 7)
(setq org-agenda-breadcrumbs-separator " ❱ ")
(setq org-agenda-todo-keyword-format "")
;; do not turn on this otherwise daily view forward/backword
;; will have issue due to some days are missing
...
Also adding icons for the category makes it look awesome:
#+begin_src emacs-lisp :tangle modules/emacs/org/config.el :mkdir yes
(setq agenda-icons-dir (concat doom-private-dir "resource/icons/"))
(after! org
(setq org-agenda-category-icon-alist
 '(
   ("agenda" "~/.doom.d/resource/icons/calendar.svg" nil nil :ascent center :mask heuristic)
   ("Birthdays" "~/.doom.d/resource/icons/cake.svg" nil nil :ascent center :mask heuristic)
   ("Holidays" "~/.doom.d/resource/icons/summer-holidays.svg" nil nil :ascent center :mask heuristic)
   ("Anniversary" "~/.doom.d/resource/icons/summer-holidays.svg" nil nil :ascent center :mask heuristic)
   ("finance" "~/.doom.d/resource/icons/money-bag.svg" nil nil :ascent center :mask heuristic)
   ("Naomi" "~/.doom.d/resource/icons/girl.svg" nil nil :ascent center :mask heuristic)
   ("inbox" "~/.doom.d/resource/icons/checklist.svg" nil nil :ascent center :mask heuristic)
   ("scheduled" "~/.doom.d/resource/icons/stopwatch.svg" nil nil :ascent center :mask heuristic)
)))
#+end_src
...
(add-to-list 'org-capture-templates
   `("m" "Meeting" entry
     (file+headline "~/org/gtd/agenda.org" "Future")
        "* %? :meeting:\n<%<%Y-%m-%d %a %H:00>>")))
#+end_src
*** capture (meeting) notes
#+begin_quote
if you capture a note while your cursor is on the same line of an agenda entry,
the content will be filled with this entry. This is super convenient to link
your note with the agenda entry. [fn:12]
#+end_quote
...
- appointment/meeting goes into ~agenda.org~
- how to surface entries tickler file?
- all project related goes into ~project.org~
- For reference, first refile into ~reference.org~, then at some point manually refile them into corresponding org file in org roam
* pomodoro
We need to add ~+pomodoro~ to the init file for org-mode.

It uses https://github.com/jwiegley/alert to display notification.
However due to [[https://github.com/julienXX/terminal-notifier][terminal-notifier]] does not support sticky notification,
I'm using [[https://github.com/vjeantet/alerter][alerter]] instead, it has very similar CLI, so we can replace it
in alert:

#+begin_src emacs-lisp :tangle modules/emacs/org/config.el
(after! alert
    (setq alert-notifier-command (executable-find "alerter")))
#+end_src

also 25 minutes pomodoro is a bit shorter for me, extend it a bit:
#+begin_src emacs-lisp :tangle modules/emacs/org/config.el
(after! org-pomodoro
...
This hook is not quite working with DOOM Emacs yet, the ~company-backends~
doesn't have ~company-org-block~.

Also due to the lazy loading of org babel language, initially only elisp is
shown up..

Another thing is it's somewhat conflicting with company-tabine
#+begin_src emacs-lisp
...
* journal
** org-journal
I found some funny issues when using DOOM emacs config for org-journal,
...
(use-package! org-journal
  ...
  (org-journal-dir "~/org/roam/journals")
  (org-journal-carryover-items nil)
  (org-journal-date-format "%a, %Y-%m-%d")
  (org-journal-search-result-date-format "%a, %Y-%m-%d")
  ;; so to make every journal entry is a first level heading
  (org-journal-time-prefix "** ")
  :config
  ;; this is to make sure this journal file is indexed by org-roam
...
;;;###autoload
(defun me/open-gtd-inbox ()
  (interactive)
  (find-file (format "%s/inbox.org" +notes-dir)))
...
** lsp
deprecated, now use pyright for lsp server..
...
#+END_SRC
** org capture rss entry
First need this function to help [fn:13]
#+begin_src emacs-lisp
;;;###autoload
(defun me/org-elfeed-entry-store-link ()
...
(add-hook 'org-store-link-functions
          'me/org-elfeed-entry-store-link)
#+end_src

Now we can define the org capture template:
#+begin_src emacs-lisp
(after! org
    (add-to-list 'org-capture-templates
      `("l" "elfeed" entry (file "~/org/gtd/inbox.org")
         "* TODO %a  :read_later: \n%U\n%?")))
#+end_src
* email
...
Update on <2021-04-01 Thu>:
Now I have changed my mind to try mu4e since Wanderlust somehow stopped working
and it also feels not very nice to use..
* json
** packages
...
#+END_SRC
** xsv.el
Sometimes csv mode can't cut it, so I wrote the xsv.el to achieve things
I can't get from csv mode with the help from xsv.

Need to install xsv first before using this package.
#+begin_src emacs-lisp :tangle modules/lang/csv/packages.el
(package! xsv :recipe (:host github :repo "emacsbliss/xsv.el"))
#+end_src
...
#+begin_src emacs-lisp
(after! lsp-java
    (setq lombok-path (substitute-in-file-name "$HOME/.m2/repository/org/projectlombok/lombok/1.18.6/lombok-1.18.6.jar"))
...
- first argument is month, January is 1
- second argument is day, Sunday is 0
- third argument is N:
...
(use-package! calendar
  ...
  '((holiday-fixed 1 1   "New Year")
    ...
    ;; Easter always occurs on the first Sunday after the Paschal Full Moon
    ;; last Monday of May
    (holiday-float 5 1 -1 "Memorial Day")
    ;; second Sunday of May
    (holiday-float 5 0 2 "Mother's day")
    ;; third Sunday of June (for US)
    ...
    (holiday-float 11 4 5 "Black Friday")
    (holiday-fixed 12 25 "Christmas")

    (holiday-fixed 1 26 "Australia Day")
    (holiday-fixed 4 25 "Anzac Day")
    ;; second Monday of June
    (holiday-float 6 1 2 "Queen's birthday (AU)")
    ;; first Monday of October
    (holiday-float 10 1 1 "Labor Day (AU)")
...
* world clock
Emacs has a built-in functionality to show a world clock by using ~M-x world-clock~

It will list a preset cities but I'd like to add more to that list..

The process is simple, already described in [fn:14]

Basically figure out the timezone name using link below:
http://en.wikipedia.org/wiki/List_of_tz_database_time_zones

Then modify the variable ~display-time-world-list~ by adding more city to it.

~display-time-world-list~ is a list of list, each inner list has the format of
~(time-zone label-for-clock)~:
#+begin_src emacs-lisp :tangle no
(push '("Asia/Shanghai" "Shanghai") display-time-world-list)
(push '("Australia/Sydney" "Sydney") display-time-world-list)
#+end_src
...
#+begin_src emacs-lisp :tangle modules/emacs/org/packages.el
(package! org-ref :recipe (:host github :branch "non-pdf-entry" :repo "emacsbliss/org-ref"))
#+end_src
...
** Org Roam Bibtex (ORB)
#+begin_src emacs-lisp :tangle modules/emacs/org/packages.el
(package! org-roam-bibtex :recipe (:host github :branch "org-roam-v2" :repo "org-roam/org-roam-bibtex"))
#+end_src
...
Sometimes it's convenient to just create a scratch buffer in order
to do some literate calculation:
#+begin_src emacs-lisp :tangle autoload/funcs.el :mkdirp yes
;;;###autoload
(defun me/literate-calc-scratch ()
  "create a scratch buffer and put in literate-calc mode"
  (interactive)
  (let ((buf (get-buffer-create (generate-new-buffer-name "*calc*"))))
    (switch-to-buffer buf)
    (org-mode)
...
(use-package! org-static-blog
  :config
    (setq org-static-blog-publish-title "EmacsBliss")
    (setq org-static-blog-publish-url "https://emacsbliss.com/")
    (setq org-static-blog-publish-directory "~/project/blog/")
    (setq org-static-blog-posts-directory "~/project/blog/posts/")
    (setq org-static-blog-drafts-directory "~/project/blog/drafts/")
...
(use-package! org-contacts
  :after org
  :custom (org-contacts-files '("~/org/roam/pages/contacts.org")))
#+end_src
* Disabled
** EAF - disabled
EAF is very unstable on macOS at the moment, also there is issue that
I can't enter text in web browser.
#+begin_src emacs-lisp :tangle no
...
[fn:12] https://github.com/rougier/emacs-gtd/blob/master/GTD.org
[fn:13] https://yiming.dev/blog/2016/01/28/add-org-store-link-entry-for-elfeed
[fn:14] http://wenshanren.org/?p=658
[fn:15] https://www.reddit.com/r/emacs/comments/hnf3cw/my_orgmode_agenda_much_better_now_with_category/
[fn:16] https://github.com/d12frosted/environment
